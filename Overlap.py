...
def getpoints():
    db_path = '/root/rosworkspace/data/pcl/mapfusion/pred_map'

    with tiledb.open(db_path, mode="r") as cloud:
        q1 = cloud.query(cond="mapid == 1")
        df1 = q1.df[:]
        src_pcd = np.asarray(df1[['x', 'y', 'z']])

        q2 = cloud.query(cond="mapid == 2")
        df2 = q2.df[:]
        tgt_pcd = np.asarray(df2[['x', 'y', 'z']])
...
if __name__ == '__main__':
    ...
    config_path = '/root/rosworkspace/src/mapfusion/overlap/configs/test/indoor.yaml'
